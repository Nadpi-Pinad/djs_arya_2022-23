...
int period=1000, flag,s , sats, Actualtime,UTCtime,UTCtime1,UTCtime2,a;
char lat_dir,lng_dirn;
uint8_t hr, mins, sec;
unsigned long time_now=0;
char storedArray[30], buffers[100];
...
void buffer_fun()
{
  ...
  rtc_loop();
  VD(); 
sprintf(buffers,"1033, %d:%d:%d, %f, %lf, %f, %lf, %02d:%02d:%02d, %lf, %lf%c, %lf%c, %d, %f,%f ",
                 UTCtime1, UTCtime2, s, realAltitude,temperature, volts, pressure,hr, mins, sec, 
                 gAltitude, latitude_val, lat_dir, longitude_val, lng_dirn, sats, tiltx, tilty); 
